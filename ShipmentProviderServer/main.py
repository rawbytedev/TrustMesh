...
def get_shipment_detail(ship_id:str):
    if ship_id in shipments:
        ...
        return {"id":ship_id, "status":"Unknow"}
## End of UI
debug = True 
...
def home(request: Request):
    return templates.TemplateResponse("index.html", {"request":request, "shipments": shipments})

@app.post("/add", response_class=HTMLResponse)
def add_shipment(request: Request, id:str =Form(...), status:str=Form(...),location:str =Form(...),notes:str=Form("")):
    shipments[id]= {"status":status, "location":location, "notes":notes}
    return templates.TemplateResponse("index.html", {"request": request, "shipments":shipments})
