    ...
    async def timer_callback(entry):
        ...
        try:
            ...
            "messages": [{"role": "user", "content": f"Set timer elapsed: {json.dumps(escrow_info)}"}]
                ...
                log.error(f"timer_callback: {e}", exc_info=True)
                log.warning("Falling back to manual handling")
        ...
        for e in batch:
            ...
            try:
                if e.etype.name in ['EXPIRED']:
                    ...
                    await toolsbase["finalize_expired_refund"].coroutine(e.escrow_id)
                if e.etype.name in ['LINKED', 'EXTENDED']:
                    log.info(f"ai_fallback: retrieving escrow details from storage")
                    _,_details = await toolsbase["get_escrow_by_id"].coroutine(e.escrow_id)
                    ...
                    if e.escrow_id == details["escrowId"]: ## sec check
                        ...
                        if state.upper() == "DELIVERED": # we trsust feed server
                            if e.etype.name in ['LINKED']:
                                # extend  we use 10s for demo
                                await toolsbase["extend_escrow"].coroutine(e.escrow_id, 15, "hold period")
                                await toolsbase["set_timer"].coroutine(e.escrow_id, 35, "release funds")
                            elif e.etype.name in ['EXTENDED']:
                                ...
                                try:
                                    ...
                                    await toolsbase["set_timer"].coroutine(e.escrow_id, "rescheduling release")
